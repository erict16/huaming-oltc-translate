...
9. 交 Word / PDF 前按 `references/word-layout.md` 做完：页眉两格表（公司名不溢出）、勾选格用嵌套两格表左右排 `□ Yes` | `□ No`、正文 Calibri 10.5 pt，图里的中文表要重画。首页页眉不得压标题行。页面上还能看见中文就还没完。
...
- [ ] 勾选格一行 `☐ Yes  ☐ No`（U+2610 选框 + Calibri 10.5 pt）；Done 列 tblGrid 加到约 2200 dxa；不是折行，不用空心几何方块 □
